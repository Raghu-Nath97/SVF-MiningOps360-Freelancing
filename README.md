...
graph TD
  %% Mobile Layer
  subgraph Mobile ["📱 MOBILE APPLICATION LAYER (Public Repo)"]
    UI[25+ Screens<br/>React Native Expo 54]
    COMP[Reusable Components<br/>SelectField, FormField, Toast]
    FORM[Validation Engine<br/>50+ Rules]
    QUEUE[Offline Queue<br/>Local Persistence]
    CLIENT_VALID[Client Validation<br/>Real-time UX]
  end

  %% Network
  UI -->|HTTPS/gRPC| API_GW

  %% API Gateway
  subgraph API ["🌐 API GATEWAY (Production Layer)"]
    API_GW[REST Router<br/>Module Endpoints]
    AUTH[RBAC Middleware<br/>Admin/Approver/Entry/Viewer]
    RATE[Rate Limiting<br/>Quota Guard]
    VALID[Request Sanitization]
  end

  %% Business Logic
  subgraph WORKFLOW ["⚙️ WORKFLOW ENGINE (Production)"]
    PENDING[Pending State Machine]
  ...
  %% Data Layer
  subgraph DATA ["💾 DATA LAYER (1,000s+ Rows)"]
    DATASTORE[(Operational Datastore<br/>Append-only<br/>Status Indexed)]
    STATUS[Status Index<br/>ACTIVE/VOID/DELETED]
  end

  %% Reporting
  subgraph REPORTING ["📊 REPORTING ENGINE"]
    ROLLUP[Auto Rollups<br/>Real-time]
    SUMMARY[Monthly + All-time Views]
    EXPORT[PDF Generation<br/>Filtered Exports]
  end

  %% Observability
  subgraph OBS ["🔭 OBSERVABILITY (Production)"]
  ...
  %% Data Flow
  UI --> FORM --> QUEUE --> CLIENT_VALID
  CLIENT_VALID --> API_GW
  API_GW --> AUTH --> RATE --> VALID --> PENDING
  PENDING --> APPROVAL --> FINALIZE --> DATASTORE
  FINALIZE --> AUDIT
  DATASTORE --> STATUS --> ROLLUP --> SUMMARY --> EXPORT
  QUEUE --> LATENCY
  UI --> CRASH
  LATENCY --> ALERTS
  ...
  classDef reporting fill:#f1f8e9
  classDef obs fill:#fce4ec

  class UI,COMP,FORM,QUEUE,CLIENT_VALID mobile
  class API_GW,AUTH,RATE,VALID api
  class PENDING,APPROVAL,FINALIZE,AUDIT workflow
  class DATASTORE,STATUS data
  class ROLLUP,SUMMARY,EXPORT reporting
  class CRASH,LATENCY,ALERTS obs
```
